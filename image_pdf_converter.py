...
class ImageToPDFConverter:
    def __init__(self, root):
        self.root = root
        self.image_paths = []  # Initialize image_paths to store selected image paths
    ...
    def select_images(self):
        # Update self.image_paths with selected images
        self.image_paths = filedialog.askopenfilenames(title="Select Images", filetypes=[("Image files", "*.png;*.jpg;*.jpeg")])
        self.update_selected_images_listbox()

    def update_selected_images_listbox(self):  # Corrected the typo in the method name
